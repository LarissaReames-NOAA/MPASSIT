module datetime_module
  ...
  ! Constant multipliers to transform a number of some time unit to
  ! another
  ...
  interface
    ...
    function c_strftime(str, slen, format, tm) bind(c, name='strftime') result(rc)
      ...
      ! format.
      ! See https://www.cplusplus.com/reference/ctime/strftime for
      ! reference.
    ...
    function c_strptime(str,format,tm) bind(c,name='strptime') result(rc)
      ...
      ! and format.
      ! See http://man7.org/linux/man-pages/man3/strptime.3.html for
      ! reference.
      import :: c_char, c_int, tm_struct
...
contains
  ...
  pure elemental subroutine tick(self)
    ! Increments the currentTime of the clock instance by one
    ! tickInterval.
    class(clock), intent(in out) :: self
  ...
  pure elemental subroutine addMilliseconds(self, ms)
    ! Adds an integer number of milliseconds to self. Called by
    ! `datetime`
  ...
  pure elemental logical function isValid(self)
    ...
    ! Returns `.true.` if the `datetime` instance is valid, and
    ! `.false.`
    ! otherwise.
    class(datetime), intent(in) :: self
  ...
  integer(int64) function secondsSinceEpoch(self)
    ! Returns an integer number of seconds since the UNIX Epoch (1 Jan
    ! 1970).
  ...
  pure elemental type(datetime) function utc(self)
    ! Returns the `datetime` instance at Coordinated Universal Time
    ! (UTC).
    class(datetime), intent(in) :: self
  ...
  pure elemental function datetime_plus_timedelta(d0,t) result(d)
    ! Adds a `timedelta` instance to a `datetime` instance, and returns
    ! a
  ...
  pure elemental logical function datetime_ge(d0,d1) result(res)
    ! `datetime` comparison operator. Returns `.true.` if `d0` is
    ! greater
  ...
  pure function datetimeRange(d0, d1, t)
    ...
    ! `datetime` instances. The number of elements is the number of
    ! whole
  ...
  pure function int2str(i, length)
    ! Converts an integer `i` into a character string of requested
    ! length,
